...
- `eden_find_workspace_items` — semantic search. Costs **one EdenAI credit per call**; say so before using it when a literal search would plausibly do.
...
- `eden_search_highlights` / `eden_list_highlights` — highlights, bounded.
...
2. **Literal before semantic.** Only reach for `eden_find_workspace_items` when literal search has failed or the request is genuinely conceptual — and disclose the credit first.
...
- `quota-exceeded` — an Eden plan or credit limit. Report it; retrying spends more.
...
- Say when a call costs something. Semantic search spends a credit; Eden MCP needs a paid Eden plan; everything sent to a tool leaves the machine and reaches a hosted US service. Mention this when the operator is choosing between approaches, not on every call.
